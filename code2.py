...
def GeneratePeople():  #generates people who need to use bathroom at random times
    ...
    while 1:
        ...
        if random.randint(0,1) == MALE:
            ...
            personNo+=1
            w2 = Label(root, image=male, borderwidth=2, bg="red")
            w2.pack(side=LEFT, padx=10)
            print "\nA male came, id=", personNo-1
        else:
            queue.insert(0,[FEMALE,personNo]);
            personNo+=1
            w1 = Label(root, image=female, borderwidth=2, bg="red")
...
if __name__ == "__main__":
    ...
    w3 = Label(root, image=bathroom, compound=CENTER).pack(side=LEFT, padx=10)
